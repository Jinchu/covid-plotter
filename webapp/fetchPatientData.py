...
def main():
    ...
    if example.fetch():
        ...
        return 7
    if len(example.total_cases) > len(example.daily_new_cases):
        warning = "WARNING!: the length of data array for "
        warning += "daily new cases is lower than for total cases."
        print(warning)
    elif len(example.total_cases) == len(example.daily_new_cases):
        print("OK: The lengths in total cases and daily new cases match")
    else:
        failure = "FAILURE he length of data array for "
        failure += "daily new cases is LARGER than for total cases."
        print("FAILURE! this step should not return success")
        return 7

    invalid_url = "http://asdfasdfasdf.ex.gov/"
    ...
    if dates[3] != 'Feb 18':
        ...
        return 9
    if sum(data) < 1010:
        print("FAILURE! The sum of found cases can't be accurate")
        return 9
    if len(test_page.find_graph_script('Total Coronavirus Death')) < 300:
